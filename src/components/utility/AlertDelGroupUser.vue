            ...
            <transition name="alertbox">
                <div
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 text-center">
                        <h2 class="text-lg">Please select Group User  before remove item </h2>
                        <h3 class="text-sm">move user to use select Group</h3>
                        <select class="form-select w-full my-3" v-model="selected_role">
                            <option :value="item.id" v-for="item in list_role" :key="item.id">{{item.name}}</option>
                        </select>
                    </div>
...
<script>
    export default {
        ...
        created() {
            this.list_role = this.$store.state.user.roles.filter(obj=>{
                return obj.id !== this.id
            })
        },
        methods:{
            submit(){
                this.$emit('confirm',this.selected_role)
            },
            close(){
                this.$emit('close')
            }
