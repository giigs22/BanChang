                ...
                <div class="main-content">
                    <div class="block-content mb-5">
                        ...
                        <div class="lg:mx-20 my-5 flex flex-col gap-3">
                            <div class="block-comp">
                                <div class="grid grid-cols-12 gap-4">
                                    <div class="col-span-12 lg:col-span-9">
                                        <div class="flex flex-col items-center justify-between lg:flex-row">
                                            <div>
                                                <h1 class="font-bold my-2 lg:text-lg text-left max-w-xl">{{$t('send_picture')}}</h1>
                                            </div>
                                            <div class="rounded-md bg-green-600 text-white px-3 py-1 lg:ml-10 lg:w-32">{{$t(comp_data.type)}}
                                            </div>
                                    ...
                                    <div class="col-span-3">
                                        <div class="flex lg:justify-end gap-2 my-3">
                                            ...
                                            <button class="bg-red-600 px-3 py-2 rounded-md font-bold">{{$t('delete')}}</button>
                                        </div>
                                ...
                                <div class="lg:ml-52 my-5 text-sm flex flex-col text-left lg:flex-row gap-2 lg:gap-5">
                                    <div><span class="font-bold">{{$t('by')}}</span> {{comp_data.name_complaint}}</div>
                                            ...
                                            <div><span class="font-bold">{{$t('date_time')}}</span> {{comp_data.date_complaint}}</div>
                                            <div><span class="font-bold">{{$t('responsible_agency')}}</span> {{comp_data.respon_agen}}</div>
                                </div>
                                <div class="bg-white-op6 py-3 px-4 rounded-md mb-5" v-if="comp_data.img_comp.length > 0">
                                    <h1 class="font-bold text-lg text-left mb-10">{{$t('reply')}}</h1>
                            ...
                            <div class="block-comp">
                                <div class="bg-cyan-400 px-3 py-2 rounded-md text-black font-bold w-32">>{{$t('reply')}}</div>
                                <textarea v-model="text_reply" rows="5" :class="error.active?'border border-red-600':''" class="my-3 dark:bg-gray-400 dark:text-white bg-wether-light rounded-xl placeholder:text-white" :placeholder="$t('type_something')"></textarea>
                                <div v-if="error.active" class="text-red-600">{{$t(error.msg)}}</div>
                                <div class="flex justify-end gap-2 my-2">
                                    <button class="bg-cyan-400 px-3 py-2 rounded-md text-black font-bold w-32" @click="submit">{{$t('submit')}}</button>
                                    <button class="bg-red-600 px-3 py-2 rounded-md font-bold w-32" @click="reset">{{$t('reset')}}</button>
                                </div>
