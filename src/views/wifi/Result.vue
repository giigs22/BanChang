            ...
            <div class="inner-content mx-4 lg:mx-10">
                <div class="main-content">
                    <div class="block-content mb-5">
                        ...
                        <h1 class="text-xl text-white ml-10">Free Wifi</h1>
                        <FilterSearch endpoint="freewifi_result" widget="wifi" @filter="emitFilter" :csv="false"></FilterSearch>
                        ...
                        <div>
                            <table class="table-auto w-full result-table">
                                ...
                                <tbody class="bg-white-op6">
                                    <tr :class="[item.status?'text-green-500':'text-red-500']"
                                        v-for="item in result_data" :key="item.device.id">
                                        ...
                                        <td>-</td>
                                        <td>{{(item.data.client)?Math.floor(item.data.client):0}}</td>
                                        <td class="border border-gray-600 text-center">{{item.status==1?'On':'Off'}}
                                        </td>
                                        <td class="border border-gray-600 text-center">
                                            {{(item.data.energy)?item.data.energy.toFixed(2):'-'}} kWh</td>
                                    </tr>
